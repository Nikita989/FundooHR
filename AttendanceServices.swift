...
class AttendanceServices: NSObject {
    ...
    func updateAttendnce(data:AttendancePopUpModel)
    {
        ...
        let params = ["token":"1a285sdffd8do8fd","engineerId":(engineerId),"timeStamp": (data.mTimeStamp),"attendanceStatus" : (data.mAttendanceStatus),"markedStatus":(data.mMarkedStatus),"punchIn":(data.mPunchIn),"punchOut":(data.mPunchOut),"reason":(data.mReason)]
