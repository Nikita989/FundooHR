...
class ProfileDataServices: NSObject {
    ...
    func updateProfileData(profilData:ProfileDataModel)
    {
        ...
        let params = ["token":"1a285sdffd8do8fd","engineerId":(engineerId),"aggregateIn": (profilData.mAggregate), "degree" : (profilData.mDegree),"diploma":(profilData.mDiploma),"discipline":(profilData.mDiscipline),"finalYearPercentage":(profilData.mFinalYearPercentage),"training":(profilData.mTraining),"trainingInstitute":(profilData.mTrainingInstitute),"trainingPeriod":(profilData.mTrainingDuration),"yearOfPassing":(profilData.mYearOfPassing)]
