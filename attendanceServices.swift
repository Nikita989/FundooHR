...
//        print("************fetchedToken",fetchedToken)
        ...
        Alamofire.request("http://192.168.0.171:3000/readEmployeeMonthlyAttendance?token=1a285sdffd8do8fd&engineerId=427188EI&timeStamp=\(timeStamp)").responseJSON
            {
            response in
...
//          let dummy = data[keys]
    ...
    func updateAttendnce(data:attendancePopUpModel)
    {
        ...
        let urlString: String = "http://192.168.0.171:3000/createEmployeeDayAttendance"
        let params = ["token":"1a285sdffd8do8fd","engineerId":"427188EI","timeStamp": (data.timeStamp),"attendanceStatus" : (data.attendanceStatus),"markedStatus":(data.markedStatus),"punchIn":(data.punchIn),"punchOut":(data.punchOut),"reason":(data.reason)]
